...
const ActionCell = ({ rowData, dataKey, onClick, ...props }) => {
  return (
    <Cell {...props} style={{ padding: '6px' }}>
      ...
      >
        {rowData.status === 'EDIT' ? 'Сохранить' : <div className="d-flex flex-column justify-content-center">
...
const LotPositionsTable = ({ data: defaultData }) => {
  ...
  return (
    <>
      {editingPosition ? <PositionEditModal isOpen={isEditModalOpen} setOpen={setEditModalOpen} position={editingPosition} setData={setData} /> : null}
      <Table height={420} data={data}>
        ...
        <Column width={120}>
          ...
          <Cell dataKey="okved_field" onChange={handleChange} wordWrap="break-all" />
        </Column>

        <Column width={100}>
          <HeaderCell>Количество, Ед. изм.</HeaderCell>
          <Cell dataKey="qty" onChange={handleChange} />
        </Column>
        <Column width={100}>
          <HeaderCell>Цена за единицу</HeaderCell>
          <EditableCell dataKey="unit_price" onChange={handleChange} />
        </Column>
        <Column width={100}>
          <HeaderCell wordWrap="break-all">Регион поставки</HeaderCell>
          <EditableCell dataKey="region" onChange={handleChange} wordWrap="break-all" />
        </Column>
